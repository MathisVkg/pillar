...
export default function IncomePage() {
  ...
  async function handleAdd(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    const source = (
      form.elements.namedItem("source") as HTMLInputElement
    ).value.trim();
    const amountExcl = parseFloat(
      (form.elements.namedItem("amountExcl") as HTMLInputElement).value,
    );
    const vatAmount =
      parseFloat(
        (form.elements.namedItem("vatAmount") as HTMLInputElement).value,
      ) || 0;
    const receivedAt = (
      form.elements.namedItem("receivedAt") as HTMLInputElement
    ).value;
    const description =
      (
    ...
    if (!source || Number.isNaN(amountExcl) || !receivedAt) {
  ...
  function statusBadgeStyle(status: string | null): React.CSSProperties {
    if (status === "waiting")
      return { background: "var(--warn-l)", color: "var(--warn)" };
    if (status === "draft")
  ...
  function ExternalForm({
    ...
    const fieldPrefix = `income-${defaultValues?.externalId ?? "new"}`;
    return (
      <div
        style={{
          background: "#fff",
          ...
          >
            ...
            >
              ...
              <input
                ...
                defaultValue={defaultValues?.amountExcl ?? ""}
                required
              />
            </div>
            <div
          ...
          >
            ...
            >
              ...
              <input
                ...
                defaultValue={
                  defaultValues
                    ? toDateInput(defaultValues.receivedAt ?? today)
                    : today
                }
                required
              />
    ...
    <div className="fp-page" style={{ maxWidth: "1100px", margin: "0 auto" }}>
      ...
      >
        <div style={{ display: "flex", gap: "6px" }}>
          <button
            type="button"
            style={chipStyle(sourceFilter === "all")}
          ...
          <button
            type="button"
            style={chipStyle(sourceFilter === "pillar")}
            onClick={() => setSourceFilter("pillar")}
          >
            {t("income.pillarOnly")}
          </button>
          <button
            type="button"
            style={chipStyle(sourceFilter === "external")}
        ...
        {(sourceFilter === "all" || sourceFilter === "external") && (
          <div style={{ display: "flex", gap: "6px" }}>
            <button
              type="button"
              style={statusChipStyle("all", statusFilter)}
            ...
            <button
              type="button"
              style={statusChipStyle("paid", statusFilter)}
            ...
            <button
              type="button"
              style={statusChipStyle("waiting", statusFilter)}
              onClick={() => setStatusFilter("waiting")}
            >
              {t("income.statusWaiting")}
            </button>
            <button
              type="button"
              style={statusChipStyle("draft", statusFilter)}
              onClick={() => setStatusFilter("draft")}
            >
              {t("income.statusDraft")}
            </button>
          </div>
        )}
      </div>
      ...
      >
        ...
        >
          ...
          >
            {displayed.length}{" "}
            {displayed.length === 1
              ? t("income.countSingular")
              : t("income.countPlural")}
          </div>
          <div
            style={{
              ...
              >
                ...
                <div data-mobile-main style={{ flex: 1, minWidth: 0 }}>
                  {entry.sourceType === "pillar" ? (
                    <>
                      ...
                      >
                        {entry.invoiceNumber ?? entry.label}
                      ...
                      >
                        {t("income.pillarInvoice")} ·{" "}
                        {t("income.paidOn").replace(
                          "{date}",
                ...
                >
                  ...
                  <div
                    style={{
                      ...statusBadgeStyle(
                        entry.sourceType === "pillar" ? "paid" : entry.status,
                      ),
                      fontSize: "10px",
                      fontWeight: 600,
                      padding: "3px 8px",
                  ...
                  {entry.sourceType === "external" && entry.externalId && (
                    <div style={{ display: "flex", gap: "4px" }}>
                      ...
                      <button
                        ...
                        onClick={() =>
                          entry.externalId && handleDelete(entry.externalId)
                        }
                      >
                        <svg
                          aria-hidden="true"
              ...
              {entry.sourceType === "external" &&
                editingId === entry.externalId && (
                  ...
                  >
                    <ExternalForm
                      onSubmit={(e) =>
                        entry.externalId && handleEdit(entry.externalId, e)
                      }
